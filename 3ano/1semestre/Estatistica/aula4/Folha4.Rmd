...
summary(modelo)
#3
#o b1 e o declive é 1.690. o intercept é a pos(0,0)e nao faz sentido vermos pinguis com bico de tamanho 0.
...
summary(modelo2)
plot(modelo2)
#apos a versao logaritmica fica mais bem comportado podemos ver iso no r squared 
predict(modelo,newdata=data.frame(carat=1.0),interval="prediction")
```
...
ggplot(gm,aes(gdpPercap,lifeExp))+
    ...
    labs(x="PIB per capita(log10 no eixo",y="Esperança media de vida")
#2
...
# No modelo lifeExp~log(gdpPercap),um aumento proporcional do PIB esta associado a um aumento aditivo na esperança de vida de magnitude igual ao declive em termos praticos dorbra o PIB per capita esta associado a 4.99 anos a mais de vida em media.
#4
augment(m_log)|>
  ggplot(aes(.fitted,.resid))+
  geom_hline(yintercept=0,linetype=2)+
  geom_point(alpha=0.7)+
  labs(x="ajustados",y="residuos",title="residuosvsajudstados")
```
...
#Interpretaçao:O declive indica o aumento medio de atraso por milha adicional.Em geral, o efeito tende a ser pequeno
#3
aug<- augment(m_dep)
cutoff<-4/nrow(fl)
