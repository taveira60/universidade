...
dados_diff<-dados_eurostat|>
  filter(year==2019)|>
  mutate(diff=abs(Female-Males)) |>
  group_by(country) |>
  summarise(media_diff=mean(diff,na.rm=TRUE)) |>
  arrange(desc(media_diff)) |>
  head(5)

ggplot(dados_diff, aes(x=contry,y=media_diff))+geom_bar(stat="identity")
...
dados_eurostat2<-dados_eurostat |>
  group_by(country) |>
  summarise(media_ratio=mean(ratio,na.rm=TRUE)) |>
  arrange(desc(media:taxa)) |>
  head(5)

eurostat_top<-dados_eurostat |>
  filter(contry %in% dados_eurostat2$country)
